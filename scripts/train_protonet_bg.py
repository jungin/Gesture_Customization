...
class_splits_file = 'class_splits_bg.json'

# --- Prepare Data Mappings ---
def prepare_split_mapping():
    mapping_all = prepare_mapping_jester(JESTER_OUTPUT_DIR, min_samples=K + Q)

    # foreground / background 분리 (gesture 이름 기준)
    fg_mapping = {k: v for k, v in mapping_all.items() if k not in BACKGROUND_CLASS_NAMES}
    bg_mapping = {k: v for k, v in mapping_all.items() if k in BACKGROUND_CLASS_NAMES}
    
    if not os.path.exists(class_splits_file):
        classes = list(fg_mapping.keys())
        random.seed(42)
    ...
    def build_mapping(cls_list):
        fg = {c: fg_mapping[c] for c in cls_list}
        bg = sum([bg_mapping[c] for c in bg_mapping], [])
        fg['bg'] = bg
        return fg
...
def main():
    ...
    # print(f"Train classes: {len(mapping_train)}")
    # print(mapping_train.keys())
    # print(f"Validation classes: {len(mapping_val)}")
    # print(mapping_val.keys())
    # print(f"Test classes: {len(mapping_test)}")
    # print(mapping_train.keys())
    ...
    # print("Validation dataset element spec:")
    # print(val_ds.element_spec)

    # print("Test dataset element spec:")
    # print(test_ds.element_spec)

    # for supp_x, supp_y, qry_x, qry_y in train_ds.take(1):
    #     print("Support Y:", supp_y.numpy())  # shape: (N*K,)
    #     print("Query Y:", qry_y.numpy())     # shape: (N*Q,)

    # for supp_x, supp_y, qry_x, qry_y in val_ds.take(1):
    #     print("Support Y:", supp_y.numpy())  # shape: (N*K,)
    #     print("Query Y:", qry_y.numpy())     # shape: (N*Q,)
    
    # for supp_x, supp_y, qry_x, qry_y in test_ds.take(1):
    #     print("Support Y:", supp_y.numpy())  # shape: (N*K,)
    #     print("Query Y:", qry_y.numpy())     # shape: (N*Q,)
    ...
    save_embeddings_to_npy(embeds, labels, prefix='stgcn_bg', out_dir='umap_outputs')

    # for CNN version
    # embeds, labels = extract_embeddings_cnn(model_cnn, test_ds)   
    # save_embeddings_to_npy(embeds, labels, prefix='cnn_bg', out_dir='umap_outputs')
